...
export function PngToJpgPage() {
  ...
  return (
    <ImageToolTemplate tool={tool}>
      {!file ? (
        <ToolUploadZone onFileSelect={handleFile} accept="image/png" label="Drop your PNG image here" sublabel="Supports PNG files" />
      ) : (
          ...
          <div className="flex gap-3 flex-wrap justify-center">
            {!result ? (
              <button onClick={() => convert(file, 'image/jpeg', quality / 100)} disabled={isProcessing} className="btn-gradient px-6 py-2.5 rounded-lg text-sm flex items-center gap-2">
                {isProcessing ? <><Loader2 size={16} className="animate-spin" /> Converting...</> : '🔃 Convert to JPG'}
              </button>
            ) : (
              <a href={result.url} download={file.name.replace(/\.png$/i, '.jpg')} className="btn-gradient px-6 py-2.5 rounded-lg text-sm flex items-center gap-2">
