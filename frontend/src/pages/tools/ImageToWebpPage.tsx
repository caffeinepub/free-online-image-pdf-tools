...
export function ImageToWebpPage() {
  ...
  return (
    <ImageToolTemplate tool={tool}>
      {!file ? (
        <ToolUploadZone onFileSelect={handleFile} accept="image/*" label="Drop your image here" sublabel="Supports JPG, PNG, GIF, BMP" />
      ) : (
        <div className="space-y-6">
          ...
          {result && <p className="text-sm text-primary text-center">✓ Converted to WebP ({(result.size / 1024).toFixed(1)} KB)</p>}
          {error && <p className="text-destructive text-sm bg-destructive/10 p-3 rounded-lg">{error}</p>}
          <div className="flex gap-3 flex-wrap justify-center">
            {!result ? (
              <button onClick={() => convert(file, 'image/webp', quality / 100)} disabled={isProcessing} className="btn-gradient px-6 py-2.5 rounded-lg text-sm flex items-center gap-2">
                {isProcessing ? <><Loader2 size={16} className="animate-spin" /> Converting...</> : '🌐 Convert to WebP'}
              </button>
            ) : (
              <a href={result.url} download={file.name.replace(/\.[^.]+$/, '.webp')} className="btn-gradient px-6 py-2.5 rounded-lg text-sm flex items-center gap-2">
