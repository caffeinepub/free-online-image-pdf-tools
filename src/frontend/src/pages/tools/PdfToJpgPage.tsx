...
export function PdfToJpgPage() {
  ...
  const { convert, isProcessing, results, error, progress, reset } =
    usePdfToJpg();
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {results.map((r) => (
                  <div key={r.page} className="space-y-2">
                    ...
                    <a
                      href={r.url}
                      download={`page-${r.page}.jpg`}
                      className="btn-gradient w-full py-1.5 rounded-lg text-xs flex items-center justify-center gap-1"
                    >
