    ...
    <ImageToolTemplate tool={tool}>
      {!file ? (
        <ToolUploadZone
          ...
          label="Drop your WebP image here"
          sublabel="Supports WebP files"
        />
      ) : (
        <div className="space-y-6">
          ...
          {result && (
            <p className="text-sm text-primary text-center">
              ✓ Converted to JPG ({(result.size / 1024).toFixed(1)} KB)
            </p>
          )}
          {error && (
          ...
          <div className="flex gap-3 flex-wrap justify-center">
            {!result ? (
              <button
                type="button"
                onClick={() => convert(file, "image/jpeg", quality / 100)}
              ...
              <a
                href={result.url}
                download={file.name.replace(/\.webp$/i, ".jpg")}
                className="btn-gradient px-6 py-2.5 rounded-lg text-sm flex items-center gap-2"
              >
