...
import { type ToolData, imageTools, pdfTools } from "@/utils/toolsData";
import { Link } from "@tanstack/react-router";
import { ArrowRight, Globe, Search, Shield, Zap } from "lucide-react";
          ...
          {(i + 1) % 5 === 0 && i < tools.length - 1 && (
            <div className="col-span-2 sm:col-span-3 lg:col-span-4 xl:col-span-5 flex justify-center py-2">
              <AdBanner
                width={728}
                height={90}
                label="Advertisement — In-Content Banner"
              />
            </div>
          )}
        </React.Fragment>
...
export function HomePage() {
  ...
  const filteredImage = imageTools.filter(
    (t) =>
      !search ||
      t.name.toLowerCase().includes(search.toLowerCase()) ||
      t.description.toLowerCase().includes(search.toLowerCase()),
  );
  const filteredPdf = pdfTools.filter(
    (t) =>
      !search ||
      t.name.toLowerCase().includes(search.toLowerCase()) ||
      t.description.toLowerCase().includes(search.toLowerCase()),
    ...
    <>
      ...
      <section className="hero-gradient relative overflow-hidden py-20 md:py-28">
        <div
          ...
          style={{
            backgroundImage:
              "url('/assets/generated/hero-bg.dim_1920x600.png')",
        ...
        <div className="container mx-auto px-4 relative z-10 text-center">
          ...
          <h1 className="font-display font-bold text-4xl md:text-6xl lg:text-7xl text-foreground mb-6 leading-tight">
            Free Online <span className="gradient-text">Image & PDF</span>
            <br />
            Tools
          </h1>
          <p className="text-muted-foreground text-lg md:text-xl max-w-2xl mx-auto mb-10 leading-relaxed">
            Compress images, convert PDFs, merge documents, and more — all free,
            fast, and secure. No uploads to servers. Everything runs in your
            browser.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center mb-10">
            <Link
              to="/tools/image-compressor"
              className="btn-gradient px-8 py-3 rounded-xl font-semibold text-sm inline-flex items-center gap-2"
            >
              Start Compressing <ArrowRight size={16} />
            </Link>
            <Link
              to="/tools/pdf-merge"
              className="px-8 py-3 rounded-xl font-semibold text-sm border border-border text-foreground hover:border-primary/50 hover:bg-primary/5 transition-all inline-flex items-center gap-2"
            >
            ...
            {[
              {
                ...
                desc: "All processing in your browser",
              },
              {
      ...
      <section className="container mx-auto px-4 py-16">
        <div className="text-center mb-10">
          ...
          <p className="text-muted-foreground max-w-xl mx-auto">
            Professional-grade tools for images and PDFs, completely free and
            browser-based.
          </p>
        </div>
        ...
        <Tabs defaultValue="image" className="w-full">
          <div className="flex justify-center mb-8">
            <TabsList className="bg-card border border-border">
              <TabsTrigger
                value="image"
                className="data-[state=active]:gradient-bg data-[state=active]:text-background font-medium"
              >
          ...
          <TabsContent value="image" className="animate-fade-in">
            {filteredImage.length > 0 ? (
              ...
              <p className="text-center text-muted-foreground py-12">
                No image tools match your search.
              </p>
            )}
          </TabsContent>
          ...
          <TabsContent value="pdf" className="animate-fade-in">
            {filteredPdf.length > 0 ? (
              ...
              <p className="text-center text-muted-foreground py-12">
                No PDF tools match your search.
              </p>
            )}
          </TabsContent>
        </Tabs>
      ...
      <div className="container mx-auto px-4 pb-8 flex justify-center">
        <AdBanner
          width={728}
          height={90}
          label="Advertisement — Between Sections"
        />
      </div>
    </>
  );
