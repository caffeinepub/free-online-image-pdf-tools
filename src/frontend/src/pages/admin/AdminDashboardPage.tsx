  ...
  return (
    <Card className="border-border">
      <CardHeader>
        ...
        <CardDescription>
          Update your admin username. Your current password is required to
          confirm the change.
        </CardDescription>
      </CardHeader>
      <CardContent>
  ...
  return (
    <Card className="border-border">
      <CardHeader>
        ...
        <CardDescription>
          Update your admin password. You'll need your current password to make
          changes.
        </CardDescription>
      </CardHeader>
      <CardContent>
  ...
  return (
    <div className="space-y-6 max-w-lg">
      ...
      <p className="text-xs text-muted-foreground px-1">
        <strong>Note:</strong> Credentials are stored in your browser session.
        They will reset to defaults if you clear your browser data or open a new
        tab.
      </p>
    </div>
  );
...
export function AdminDashboardPage() {
  ...
  const { data: analyticsData = [], isLoading } =
    useMultipleDayAnalytics(last7Days);

  const totalVisitors = analyticsData.reduce(
    (sum, d) => sum + Number(d.visitorCount),
    0,
  );
  const todayData = analyticsData.find(
    (d) => d.date === last7Days[last7Days.length - 1],
  );
  ...
  for (const day of analyticsData) {
    for (const tu of day.toolUsage) {
      toolUsageMap[tu.toolName] =
        (toolUsageMap[tu.toolName] || 0) + Number(tu.usageCount);
    }
  }
  ...
  return (
    <>
      <SEOHead
        title="Admin Dashboard - ToolsHub Free"
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1">
              Analytics & site performance overview
            </p>
          </div>
          <button
            type="button"
          ...
          <TabsContent value="analytics">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
              ...
              <StatCard
                ...
                label="Page Views"
                value={(totalVisitors * 2.4).toFixed(0)}
                sub="~2.4 pages/visit"
              />
              <StatCard
                icon={<MousePointer size={18} className="text-primary" />}
                label="Ad Clicks (Est.)"
                value={adClickSim.toLocaleString()}
                sub="~3% CTR simulation"
              />
            </div>
                ...
                {isLoading ? (
                  ...
                  <ResponsiveContainer width="100%" height={200}>
                    ...
                    >
                      <CartesianGrid
                        strokeDasharray="3 3"
                        stroke="oklch(0.28 0.025 265)"
                      />
                      <XAxis
                        dataKey="date"
                        tick={{ fill: "oklch(0.6 0.03 265)", fontSize: 11 }}
                      />
                      <YAxis
                        tick={{ fill: "oklch(0.6 0.03 265)", fontSize: 11 }}
                      />
                      ...
                      <div key={name} className="space-y-1">
                        ...
                        <div className="h-1.5 bg-muted rounded-full overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${totalToolUsage > 0 ? (count / sortedTools[0][1]) * 100 : 0}%`,
                            }}
                          />
                        </div>
                      </div>
            ...
            <div className="bg-card border border-border rounded-xl p-6">
              ...
              {sortedTools.length === 0 ? (
                <p className="text-muted-foreground text-sm">
                  No data available. Tool usage will appear here as users
                      ...
                      {sortedTools.map(([name, count], i) => (
                        ...
                        >
                          ...
                          <td className="py-2.5 text-right text-primary">
                            {totalToolUsage > 0
                              ? ((count / totalToolUsage) * 100).toFixed(1)
                              : 0}
                            %
                          </td>
                        </tr>
